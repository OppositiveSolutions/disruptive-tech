...
public interface QuestionPaperSubCategoryRepository extends JpaRepository<QuestionPaperSubCategory, Integer> {
    ...
    @Query("SELECT new com.careerfocus.model.response.QPSubCategoryVO(q.questionPaperSubCategoryId, q.direction, " +
            "q.description, q.noOfQuestions) FROM QuestionPaperSubCategory q WHERE q.questionPaperCategoryId=?1")
