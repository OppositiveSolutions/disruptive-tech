...
public interface StudentRepository extends JpaRepository<Student, Integer> {
    ...
    @Query(value = "SELECT new com.careerfocus.model.response.StudentVO(u.userId, CONCAT(u.firstName, ' ', u.lastName), "
            ...
            + "LEFT JOIN u.userPhones p WHERE (p.isPrimary=1 OR p.isPrimary IS NULL) AND u.role = 1"
    ...
    @Query(value = "SELECT new com.careerfocus.model.response.StudentVO(u.userId, CONCAT(u.firstName, ' ', u.lastName), "
            ...
            + "WHERE (p.isPrimary=1 OR p.isPrimary IS NULL) AND u.role = 1 AND (LOWER(CONCAT(u.firstName, ' ', u.lastName)) "
